...
echo "步骤 2: 模拟用户场景"
echo "----------------------------------------"
echo "场景：用户执行一个失败的命令，然后直接使用 diu"
echo ""
echo "示例命令序列："
echo "  1. ls -l /nonexistent     (执行失败的命令)"
echo "  2. diu                    (直接使用 diu，无需手动输入)"
echo ""
...
echo "  2. diu                   (运行 diu)"
echo "预期：diu 应该能够获取到上一条命令 'echo test command'"
echo ""
...
echo "  2. diu                   (运行 diu)"
echo "预期：diu 应该跳过重放，直接将命令传递给 diu-core"
echo ""
...
echo "  2. diu                   (运行 diu)"
echo "预期：diu 应该捕获到错误信息并传递给 diu-core"
echo ""
